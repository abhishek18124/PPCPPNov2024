...
bool isValid(int grid[][9], int n, int i, int j, int d) {
	...
	for (int k = 0; k < n; k++) {
		if (grid[i][k] == d || grid[k][j] == d) {
			// d cannot be assigned to the i,jth cell since
			// it present in the ith row or jth column
			return false;
		}
	}

	// d is not present in the ith row or jth column therefore now

	// check if d is present in the subgrid of i,jth cell or not
	...
	for (int x = sx; x < sx + rn; x++) {
		for (int y = sy; y < sy + rn; y++) {
			if (grid[x][y] == d) {
				// d cannot be assigned to the i,jth cell
				// since it is present in the subgrid of
				// i,jth cell
				return false;
			}
		}
	...
	return true; // d can be assigned to the i,jth cell
	// since it is not present in the ith row or jth col
	// or the subgrid in which i,jth cell is present
